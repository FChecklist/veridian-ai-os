...
- [x] First push (`cd398e2`) surfaced 3 real CI failures, not false alarms: Type Check (TS18048, `Boolean(delegator) &&` doesn't narrow past the call), Metadata Index Coverage Check (the PR's own pre-existing `ai-os/DELEGATION_EXPIRY_ENFORCEMENT_AUDIT_2026-07-26.md` was never indexed in `ai-os/OS.yaml` -- that gate didn't exist yet when this PR's original commits landed, 1039 commits behind current main), Terminology Guardrail Check (this fix's own 4 new dated gap-closure comments tripped `hardcoded_iso_date`, unexempted).
- [x] Fixup commit pushed (`0073162`, parent `cd398e2`): `delegator != null &&` instead of `Boolean(delegator) &&` in both consumers; real `covers` entry added to `ai-os/OS.yaml` for the pre-existing audit doc; real exemption entries added/bumped in `ai-os/registry/terminology-guardrail-exemptions.yaml` (same pattern that file already uses for every other dated gap-closure comment in this codebase).
- [ ] Real independent audit: pending, after CI is confirmed green on the fixup commit.
- [ ] CI: re-running on `0073162` as of this checkpoint.
