## OBJECTIVE
Serverless resource-limit tradeoff doc + heavy-workload audit
...
ai-os/TIER3_RELEVANCE_TRIAGE_REPORT_2026-07-26.md, objective key `serverless-resource-limit`) independently
...
---
TASK: V2-12 -- Serverless resource-limit tradeoff doc + heavy-workload audit
...
No doc found anywhere in ai-os/ for this item; `ai-os/GAP_ANALYSIS_2026-07-20_HOLD.md` explicitly still lists "V2-12 (serverless resource-limit doc)" under the still-held Low bucket. No `maxDuration` config anywhere in src/app, no new queue/worker infra. No git commits, no ACTIVE-CLAIMS/COMPLETED entries since.

Nothing has closed this since the pre-flight block; the gap analysis doc itself still lists it open.
...
before opening the PR, and again in the PR description):
    ...
    bash -c 'find ai-os -iname "*serverless*resource*" -o -iname "*serverless*limit*"'  # doc must exist and list any workload fixes made
